    ...
    def _build_user_embed(self) -> discord.Embed:
        ...
        embed.add_field(
            ...
            value=(
                ...
                f"  - **동전던지기(앞/뒤)**: 3{CURRENCY} 베팅 → 맞추면 **9{CURRENCY}**, 틀리면 추가 6 차감(총 **-9{CURRENCY}**)\n"
                f"  - **주사위(1회)**: 2{CURRENCY} 베팅, 적중 **12{CURRENCY}**, 실패 -2\n"
                f"  - **주사위(2회)**: 5{CURRENCY} 베팅 → 하나 적중 **18{CURRENCY}**, 둘 다 적중 **180{CURRENCY}**, 둘 다 실패 **-10{CURRENCY}**\n"
                f"  - **주사위(3회)**: 25{CURRENCY} 베팅 → 1개 적중 **72{CURRENCY}**, 2개 **360{CURRENCY}**, 3개 **5400{CURRENCY}**, 모두 실패 **-50{CURRENCY}**"
            ),
            inline=False,
        )
